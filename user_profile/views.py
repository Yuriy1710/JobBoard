...
from user_profile.models import ConversationMessage
...
def view_application(request, application_id):
    ...
    if request.method == "POST":
        ...
        if content:
            conversation_message = ConversationMessage.objects.create(application=application, content=content, created_by=request.user) 
            return redirect('view_application', application_id=application_id)
            
    return render(request, 'view-application.html', {'application': application})


@login_required
def view_dashboard_job(request, job_id):
    job = get_object_or_404(Job, pk=job_id, created_by=request.user)
    return render(request, 'view-dashboard-job.html', {'job': job})